...
### Created by Anton Hughes ( magick93 ) on 2025-05-23
...
readonly script_author="anton.hughes@sigmaprime.io"
...
flag|q|quiet|Suppress non-essential output
flag|v|verbose|Enable debug output
flag|f|force|Skip confirmation prompts
flag| |generate|Generate support bundle

option|c|kubeconfig|Path to kubeconfig file|$HOME/.kube/config
option|n|namespace|Longhorn namespace|longhorn-system
option|o|output|Output format (text/json)|text
option| |timeout|Command timeout in seconds|30
option| |cluster|Cluster context to use|current

param|1|action|Action to perform: check-system/check-components/check-volumes/support-bundle
" | grep -v '^#' | grep -v '^\s*$'
}
...
main() {
  ...
  case $action in
  check-system)
    #TIP: use «$script_prefix check-system» to validate cluster requirements
    do_check_system
    ;;

  check-components)
    #TIP: use «$script_prefix check-components» to inspect Longhorn components
    do_check_components
    ;;

  check-volumes)
    #TIP: use «$script_prefix check-volumes» to diagnose storage volumes
    do_check_volumes
    ;;

  support-bundle)
    #TIP: use «$script_prefix support-bundle» to generate diagnostic bundle
    do_generate_bundle
    ;;
...
do_check_system() {
  log_to_file "Checking system requirements"
  local exit_code=0
  
  # 1. Check Kubernetes version
  local k8s_version
  k8s_version=$(kubectl version -o json 2>/dev/null | jq -r '.serverVersion.gitVersion')
  [[ -z "$k8s_version" ]] && die "Could not connect to Kubernetes cluster"
  out "${col_grn}✓${col_reset} Kubernetes Version: ${k8s_version%%-*}"

  # 2. Validate nodes meet Longhorn requirements
  out "\n${col_ylw}=== Node Hardware Checks ===${col_reset}"
  kubectl get nodes -o json | jq -r '.items[] | .metadata.name + " " + .status.capacity.cpu + " CPUs " + .status.capacity.memory' | while read -r node; do
    local node_name=$(echo "$node" | awk '{print $1}')
    local cpu=$(echo "$node" | awk '{print $2}')
    local mem=$(echo "$node" | awk '{print $3}')
    
    if [[ $cpu -lt 4 ]]; then
      out "${col_red}✗${col_reset} $node_name: Insufficient CPUs ($cpu < 4 minimum)"
      exit_code=1
    else
      out "${col_grn}✓${col_reset} $node_name: CPU cores - $cpu"
    fi
  done

  # 3. Check for SSD/NVMe storage devices via node labels
  out "\n${col_ylw}=== Storage Checks ===${col_reset}"
  kubectl get nodes -o json | jq -r '.items[].metadata.name' | while read -r node; do
    local disk_type
    disk_type=$(kubectl get node "$node" -o json | jq -r '.metadata.labels["node.longhorn.io/default-disk-type"]')
    if [[ "$disk_type" == "ssd" || "$disk_type" == "nvme" ]]; then
      out "${col_grn}✓${col_reset} $node: SSD/NVMe storage configured ($disk_type)"
    else
      out "${col_red}✗${col_reset} $node: No SSD/NVMe storage detected (found '${disk_type:-none}')"
      exit_code=1
    fi
  done
...
do_check_components() {
  ...
  local ui_status
  ui_status=$(kubectl get deployment -n "$namespace" longhorn-ui -o json | jq -r '.status.conditions[] | select(.type == "Available").status')
  if [[ "$ui_status" == "True" ]]; then
    out "${col_grn}✓${col_reset} UI Deployment: Available"
  else
    out "${col_red}✗${col_reset} UI Deployment: Unavailable"
    exit_code=1
  fi
...
show_usage() {
  out "${col_grn}Longhorn Diagnostic Toolkit ${col_reset}v$script_version"
  out "${col_ylw}Usage:${col_reset} $script_basename [options] <action>"
  out ""
  out "${col_ylw}Actions:${col_reset}"
  out "  check-system     Validate cluster hardware/k8s requirements"
  out "  check-components Inspect Longhorn components and status"
  out "  check-volumes    Diagnose storage volume issues"
  out "  support-bundle   Generate cluster diagnostics bundle"
  out ""
  out "${col_ylw}Examples:${col_reset}"
  out "  # Basic cluster check:"
  out "  $script_basename check-system"
  out ""
  out "  # JSON output for automation:"
  out "  $script_basename check-components --output json"
  out ""
  out "  # Custom kubeconfig and namespace:"
  out "  $script_basename check-volumes --kubeconfig ./prod-cluster.yaml --namespace longhorn-prod"
  out ""
