...
for lvmd_setting in "true" "false"; do
    ...
    gist_url=$(gh gist create -p -d "[blockprint] TopoLVM v15.5.5 Debug Output (lvmdEmbedded=$lvmd_setting) v15.0.0" -f "topolvm-debug-lvmdEmbedded-$lvmd_setting.txt" <<< "$debug_output")
